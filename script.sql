...
BEGIN TRY
	BEGIN TRANSACTION
			...
			IF @saldo_antes_enviador IS NULL
				THROW 50001,'Conta de origem n�o existente',1
			...
			IF @data_atual IS NULL
				...
				IF @limite_pedido < @valor+@valor_hoje
					THROW 50007,'Limite di�rio excedido',1
			...
			IF @conta_enviado IS NULL
				THROW 50003,'Conta de destino n�o existente',1
...
SET NOCOUNT ON	
	...
	IF EXISTS (SELECT 1 FROM usuario WHERE email = @email OR cpf = @cpf)
		THROW 50004,'Usu�rio j� cadastrado',1
...
BEGIN TRANSACTION
	...
	IF @@ROWCOUNT = 0
		THROW 50005,'Conta n�o encontrada',1
	COMMIT
	END TRY
BEGIN CATCH
	IF @@TRANCOUNT > 0
...
BEGIN TRY
	...
	IF NOT EXISTS (SELECT 1 FROM transacao WHERE id_enviador = @id_conta OR id_enviado = @id_conta)
		THROW 50006,'Nenhuma transa��o realizada.',1
	SELECT 
            t.data_hora,
            t.valor,
            CASE 
...
BEGIN TRANSACTION
	...
	IF @saldo > 0
		THROW 50008, 'N�o � poss�vel fechar conta com saldo positivo', 1
...
BEGIN TRANSACTION
	...
	IF EXISTS (SELECT 1 FROM conta WHERE id_usu = @id_usu AND ativo = 1)
		THROW 50009,'Feche a conta antes de apagar o usu�rio',1
	...
	SET NOCOUNT ON
		BEGIN TRY
			IF @email IS NULL OR @senha IS NULL
				THROW 500010,'Dados inv�lidos',1
